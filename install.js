...
function testBinary(binaryPath) {
  try {
    ...
    if (result.stderr && result.stderr.includes("GLIBC_")) {
      console.warn(
        `⚠️  Binary compatibility issue: ${result.stderr.split("\n")[0]}`,
      );
      return false;
    }
...
function install() {
  ...
  if (platform === "linux") {
    ...
    if (existsSync(muslBinaryPath)) {
      console.log(
        "📦 Found musl binary, which should work on most Linux systems",
      );
    ...
    // Fall back to detected libc type
      ...
      if (!PLATFORMS[platform][arch][libcType]) {
        ...
        console.warn(
          `⚠️  Detected libc type is not supported, trying ${libcType} instead`,
        );
      }
    ...
    if (!existsSync(binaryPath)) {
      ...
      const alternativeBinaryPath = join(
        __dirname,
        "bin",
        platform,
        arch,
      ...
      if (existsSync(alternativeBinaryPath)) {
        console.warn(
          `⚠️  Binary for ${libcType} not found, trying ${alternativeLibc} instead`,
        );
    ...
    if (platform === "linux") {
      ...
      if (!testBinary(targetPath)) {
        ...
        if (binaryName.includes("-gnu")) {
          ...
          if (existsSync(muslBinaryPath)) {
            console.log(
              "🔄 GNU binary not compatible, trying musl binary instead",
            );
            ...
            if (!testBinary(targetPath)) {
              console.error(
                "❌ Both GNU and musl binaries failed to run on this system.",
              );
            ...
            console.error(
              "❌ GNU binary not compatible, and musl binary not available.",
            );
            reportCompatibilityError();
            process.exit(1);
          }
...
function reportCompatibilityError() {
  console.error("\n🔧 Possible solutions:");
  console.error(
    "1. Try using the musl binary, which has fewer system dependencies:",
  );
...
if (process.env.FORCE_MUSL === "1" && platform === "linux") {
  console.log(
    "🔧 FORCE_MUSL=1 environment variable detected, forcing musl binary",
  );
